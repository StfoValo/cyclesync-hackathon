...
function renderTelemetry(tel) {
    ...
    if (!tel || !tel.vin) { grid.innerHTML = '<div class="text-slate-500 text-sm col-span-4">No telemetry data</div>'; return; }

    let html = _cell('Odometer', tel.current_odometer_km ? `${tel.current_odometer_km.toLocaleString()} km` : '—') +
        ...
        _cell('Coolant Temp', tel.coolant_temp_c != null ? `<span class="${tel.coolant_temp_c > 95 ? 'text-amber-400' : 'text-emerald-400'}">${tel.coolant_temp_c}°C</span>` : 'N/A');

    // TPMS
    if (tel.tpms_fl != null) {
        html += `<div class="bg-slate-800/50 rounded p-3 border border-white/5 sm:col-span-2 lg:col-span-4">
            <p class="text-xs text-slate-400 mb-2">TPMS Pressure (bar)</p>
            <div class="grid grid-cols-4 gap-2">${['fl','fr','rl','rr'].map(p => {
                const v = tel[`tpms_${p}`]; const c = v < 2.0 ? 'text-rose-400' : v < 2.2 ? 'text-amber-400' : 'text-emerald-400';
                return `<div class="text-center"><div class="text-xs text-slate-500 mb-1">${p.toUpperCase()}</div><div class="font-bold ${c}">${v?.toFixed(2) ?? '—'}</div></div>`;
            }).join('')}</div></div>`;
    }

    // Battery
    if (tel.battery_soc != null) html += _cell('Battery SoC', `${tel.battery_soc}%`);
    if (tel.battery_soh != null) html += _cell('Battery SoH', `${tel.battery_soh}%`);

    // Blackbox sensors
    if (tel.has_blackbox) {
        html += _cell('Accel X', tel.accel_x_g != null ? `${tel.accel_x_g}g` : '—') +
            _cell('Accel Y', tel.accel_y_g != null ? `${tel.accel_y_g}g` : '—') +
            _cell('GPS', tel.gps_lat != null ? `${tel.gps_lat?.toFixed(4)}, ${tel.gps_lon?.toFixed(4)}` : '—') +
            _cell('Heading', tel.gps_heading_deg != null ? `${tel.gps_heading_deg}°` : '—');
    }

    // Engine sensors
    if (tel.oil_temp_c != null) html += _cell('Oil Temp', `${tel.oil_temp_c}°C`);
    if (tel.engine_rpm != null && tel.engine_rpm > 0) html += _cell('Engine RPM', `${Math.round(tel.engine_rpm)}`);
    if (tel.throttle_position_pct != null) html += _cell('Throttle', `${tel.throttle_position_pct}%`);
    if (tel.fuel_level_pct != null) html += _cell('Fuel Level', `${tel.fuel_level_pct}%`);

    html += _cell('Last Sync', tel.last_sync_timestamp ? `<span class="font-mono text-xs">${new Date(tel.last_sync_timestamp).toLocaleString()}</span>` : '—');
    grid.innerHTML = html;
}
